...
function vartest( x, y, options ) {
	...
	if ( !isTypedArrayLike( x ) && !isNumberArray( x ) ) {
		throw new TypeError( format( '1Lv8R', x ) );
	}
	if ( !isTypedArrayLike( y ) && !isNumberArray( y ) ) {
		throw new TypeError( format( '1Lv9k', y ) );
	}
	opts = {};
	if ( options ) {
	...
	if ( alpha < 0.0 || alpha > 1.0 ) {
		throw new RangeError( format( '1Lv8V', 'alpha', alpha ) );
	}
	dfX = x.length - 1;
	dfY = y.length - 1;
	...
	case 'less':
		...
		break;
	default:
		throw new Error( format( '1Lv4S', 'alternative', [ 'two-sided', 'less', 'greater' ].join( '", "' ), alt ) );
	}
	out = {};
